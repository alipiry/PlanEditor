...
void ParametersTreeModel::Save(QString fileName)
{
    ...
    for (int i=0; i<=treeItemData->childCount(); i++) {
        ...
        variant = index(i, 0, QModelIndex()).data();
        file << variant.toString().toStdString()<<"\n";

        int j = 0;
        if (index(j, 0, indexA).isValid())
            while(index(j, 0, indexA).data().isValid()) {
                variant = index(j, 0, indexA).data();
                if (variant.toString().toStdString() == index(0, 0, QModelIndex()).data().toString().toStdString())
                    break;
                file << "  "<<variant.toString().toStdString()<<"\n";
